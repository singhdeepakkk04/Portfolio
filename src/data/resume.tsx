...
export const DATA: ResumeData = {
  ...
  summary:
    "I manage a portfolio of three to four product initiatives at a time. One has shipped, a custom **EMR platform** for eye-care hospitals is in UAT, two more are in build, and another begins shortly. On the EMR I am the sole PM: twenty-odd discovery sessions with ophthalmologists and clinical staff, translated into epics, features and user stories carrying the acceptance criteria and definition of done an engineering team could build against.\n\nAt **MAQ Software** I own the reporting roadmap for product and revenue metrics, set the measurement approach across those initiatives, and run the experimentation practice behind each product bet. Much of the role is translation: converting an ambiguous commercial question from Sales or Operations into a specification engineering can estimate, then holding scope and schedule once delivery is underway. One automation I drove returned **80+ analyst-hours a month** to a team that had been doing the work by hand.\n\nI work where the commercial question meets the data model, and I am equally at home on either side of it. When a Head of Sales asks for a number, the useful answer covers three things: whether that metric is the right proxy for the decision being made, whether the grain of the underlying tables supports the cut being requested, and what pipeline freshness and SLA mean for how far the figure should be trusted that morning. **Twilio Segment** and **Azure Data Engineer** certifications formalise part of that; three years owning production data platforms at **LTIMindtree** is where the rest of it came from. That is not a background I moved on from, it is half of how I make product decisions.\n\nI tend to spot a gap and then spend weekends closing it. Most recently: practising Business Analysts lose roughly half their document work to templates. I interviewed a group of them to confirm the pattern held, then built a **13-agent RAG system** that drafts those deliverables, fine-tuned on real BA artefacts. Product manager, architect and sole engineer on that one.",
  avatarUrl: "/mee.jpg",
  skillGroups: [
    {
